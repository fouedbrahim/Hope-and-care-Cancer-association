		...
		<section class="row-fluid">
			<section id="donation_box">
				<section class="container container-fluid">
					<section class="row-fluid">
					<div class="span8 first" > <h2>  Administration~administrateurs  </h2> </div>
		...
		<section class="row-fluid">
				...
				<section class="span9" id="blog_listing">
					...
					<div class="inner_lyr">
						...
						<p ><h4 align="center"> Liste des administrateurs</h4> </p><br><br>
		<form method="post">		
	<div id="rech"><input type="text" name="rech"><input type="submit" value="Rechercher" id="button3"> </div>
	<br><br>
</form>	 		
<?php
include("connexion.php");
$qry='select * from utilisateur where type="admin" ';
	
  if(empty($_POST['rech'])==false)
 {$qry='select * from utilisateur where type="admin" and (nom LIKE "%'.$_POST['rech'].'%" or prenom LIKE "%'.$_POST['rech'].'%" or email LIKE "%'.$_POST['rech'].'%");';}
 //mysql_query($qry);
$resultat = mysql_query($qry);
$nb =mysql_num_rows($resultat);
if($nb>0)
{echo'<table border="2" align="center" id="tab">'
?>

<tr><td>Photo</td><td>Nom </td><td>Prenom</td><td>Sexe</td> <td>Date de naissance</td><td>Adresse</td><td>Telephone</td><td>Email</td><td>Modifier</td><td>Supprimer</td></tr>
...
while($ligne=(mysql_fetch_row($resultat)))
		...
		?>
			<tr>
					<td width="100" height="60"><img src="./photo/<?php echo $ligne[8] ?>" ></td>
					...
					<td><a href="modifier_admin.php?email=<?php echo $ligne[6]?>&nom=<?php echo $ligne[0]?>&prenom=<?php echo$ligne[1]?>&sexe=<?php echo$ligne[2]?>&daten=<?php echo$ligne[3]?>&adresse=<?php echo$ligne[4]?>&tel=<?php echo$ligne[5]?>&photo=<?php echo$ligne[8]?>" >modifier</a>
					<td><a href="supp_admin.php?email=<?php echo $ligne[6]?>" >supprimer</a>
	</tr>
	<?php
	}}else echo' <br> <h3 align="center">Table vide</h3>'; echo'</table>'
	?>
